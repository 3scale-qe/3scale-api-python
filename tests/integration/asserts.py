...
def assert_resource(resource: threescale_api.defaults.CRUDResource):
    assert resource is not None
    assert resource.entity_id is not None
    assert resource.entity is not None


def assert_errors_contains(resource: threescale_api.defaults.CRUDClient, fields: List[str]):
...
def assert_resource_params(obj: threescale_api.defaults.CRUDResource, params: dict, allowed=None):
